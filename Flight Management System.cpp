...
const char l_destination[5][20]={"ISTANBUL","ANKARA","IZMIR","ANTALYA","KONYA"};	//local Destination
const char i_destination[11][20]={"LONDON","BERLIN","DUBAI","RIYADH","BEIJING","PARIS","ISTANBUL","TOKYO","WASHINGTON","DELHI","MOSCOW" }; //International Destination
		...
		bool idest(string pass)
	{
		...
		else
		{
			...
			if(m==1&&n==1)
			{
				...
				cout<<endl<<"Price: "<<payment<<" TL/seat"<<endl;
				return true;
			}
			else 
				return false;
		...
		void Registration()
		{
			...
			if(k==49)
			{
				...
				<<endl<<"Time: 4.00 PM"
				<<endl<<"Ticket: TR"<<tkt
				<<endl<<"Bill(in TURKISH LIERAS): "<<payment<<"/-"<<endl;				
				outfile.close();
			}
			else if(k==50)
			{
...
int main()
{
	...
	cout<<endl<<"\n\n\n\t\t\t\t\t  Welcome to TURIN Airlines"<<endl;
